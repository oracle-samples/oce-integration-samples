...
define(['knockout',
	...
	var SampleComponentViewModel = function (args) {
		...
		// If we're on a development system then we have to switch our authentication method
		if (window.location.port === '8080' && (window.location.hostname.indexOf('cec.oraclecorp.com') !== -1)) {
			var userCredentials = window.localStorage['scs.component.userCredentials'] || 'swebcli.SitesAdmin1.admin:Welcome1';
			self.authorization = 'Basic ' + window.btoa(userCredentials);
		}
		...
		function getBearerToken() {
			...
			return new Promise(function (resolve) {
				$.ajax({
					type: 'GET',
					url: `/documents/web?IdcService=GET_OAUTH_TOKEN`,
					headers: {
						Authorization: self.authorization,
					},
					success: function (data) {
						resolve(data.LocalData.tokenValue);
					},
					error: function () {
						console.error("Could not get Bearer Token using IdcService API.");
					}
				});
			});
		...
		function getData() {
			// Check if it is a secure site
			if (SCS.siteInfo.properties.siteRootPrefix.includes('authsite'))
		...
		self.updateCustomSettingsData = $.proxy(function (customData) {
			if (customData) {
				...
				// Get necessary data and then load the chatbot
				getData();
			}
		}, self);
