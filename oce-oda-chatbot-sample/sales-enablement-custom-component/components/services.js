...
function createHTML(temp, fileType) {
  let obj = '';
  obj += `<div class="SE2-Story-readNext" title="${temp.title}">`;
  obj += '<div class="contentWrapper">';
  if (fileType === 'text') {
    obj += '';
  } else if (fileType === 'mp4') {
    obj += `<div class="imgDiv"><video controls><source src="${temp.url}"></video></div>`;
  } else {
    obj += `<div class="imgDiv"><img src="${temp.url}" alt="${temp.title}"></div>`;
  }
  obj += `<div class="${temp.textDiv}">`;
  obj += '<div class="AuthorTitle">';
  obj += `<div class="font14 font14-2LineEllipsis Title">${temp.title}</div>`;
  obj += '<div class="font12 Author">';
  obj += `<span class="font16-1LineEllipsis">${temp.createdbyname}</span>`;
  obj += `<span class="Date">${temp.updatedDate}</span>`;
  obj += '</div>';
  obj += '</div>';
  obj += `<div class="font12 font12-2LineEllipsis Summary">${temp.summary}</div>`;
  obj += '</div>';
  obj += '</div>';
  obj += '</div>';
  return obj;
